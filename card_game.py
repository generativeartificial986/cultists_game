...
CARD_DEFINITIONS = {
    ...
    "Violent Delights": {
        ...
        "description": "Participate directly or indirectly (such as by voting) in the death of another player within three rounds of playing this card, and earn two extra cards during the following Morning. However, failing to kill anyone will cause you to lose two random cards!",
        "phase_restriction": ["Evening"],
        "target_type": "self",
...
class Player:
    ...
    def decrement_status_effects(self):
        ...
        for effect in effects_to_remove:
            ...
            print(f"{self.name}'s {effect} effect has worn off.")
    ...
    def advance_phase(self):
        ...
        if self.current_phase == "Lobby":
            ...
            if self.apocalypse_vote_target:
                ...
                self.public_announcements.append("Night has fallen. All players must now close their eyes and sleep.")
            ...
            self.current_phase = "Night"
            self.public_announcements.append("The vote is over. Night has fallen. All players must now close their eyes and sleep.")
            for pid in self.players:
                self.players[pid].decrement_status_effects()
            for pid in self.players: self.players[pid].is_asleep = False
            self.night_asleep_players.clear()
            ...
            self.voting_sub_phase = "Inactive"
            self.public_announcements.append("The vote is over. You may now play special cards before night falls.")
            self.dusk_ready_players.clear()
        elif self.current_phase == "Dusk":
            self.current_phase = "Evening"
            self.last_phase_start_time = time.time() # FIXED: Reset timer for the new Evening
            ...
            for pid in self.alive_players: self.players[pid].has_submitted_evening_cards = False
