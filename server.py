...
def handle_disconnect():
    ...
    if player:
        ...
        game_state.lobby_ready_players.discard(pid) # MODIFIED: Remove from ready set on disconnect
...
def handle_submit_evening_cards(data):
    ...
    all_players_with_actions = game_state.alive_players + [pid for pid in game_state.dead_players if game_state.players[pid].hand]
    if game_state.current_phase == "Evening" and len(game_state.evening_submitted_players) >= len(all_players_with_actions):
...
def handle_play_voting_card(data):
    sid = request.sid
    pid = clients.get(sid)
    if not pid or game_state.current_phase != "Voting":
        return
...
def apply_card_effect(player_id, card_obj, target_list=None):
    ...
    if t1_obj and 'immolated' in t1_obj.status_effects and player_id != t1_obj.player_id:
        if not 'burning' in player.status_effects:
            ...
            game_state.delayed_actions.append({
                'type': 'burn_death',
                'target_id': player_id,
                'source_id': t1_obj.player_id,
                'execute_at_round': game_state.round_number + 2
            })
            game_state.public_announcements.append(f"{player.name} caught fire! They will die within two rounds unless saved.")
            print(f"[EFFECT] {player.name} caught fire from attacking {t1_obj.name}. Will die in round {game_state.round_number + 2}")
    ...
    if card_obj.effect_type == "mark_of_the_beast":
        ...
        if t1_obj and t1_obj.is_alive:
            ...
            print(f"[CARD] {player.name} played 'Oh God, Please! Anything But This!' on {t1_obj.name}")
        ...
        player.apply_status_effect("silence", card_obj.duration_rounds)
        player.apply_status_effect("delirium", card_obj.duration_rounds)
        ...
        player.apply_status_effect("silence", card_obj.duration_rounds)
        player.apply_status_effect("delirium", card_obj.duration_rounds)
        player.apply_status_effect("vote_restriction", card_obj.duration_rounds)
        ...
        if t1_obj:
            ...
            t1_obj.apply_status_effect("divine_protection", card_obj.duration_rounds)
        ...
        if action['execute_at_round'] == game_state.round_number:
            if action['type'] == 'burn_death':
                ...
                if thief and victim and victim.is_alive and victim.hand:
                    ...
                    game_state.public_announcements.append(f"{victim.name} had a card stolen! Be on the lookout for a thief...")
                    print(f"[EFFECT] {thief.name} stole {stolen_card.name} from {victim.name}")
                else:
...
def kill_player(player_id, source, killers=[]):
    ...
    if pl and pl.is_alive:
        ...
        for c in game_state.dead_deck.deal(3): pl.add_card(c)
...
def game_loop():
    ...
    while True:
        ...
        if (phase == "Evening" and now - start_time >= EVENING_TIMER_SECONDS):
            game_state.advance_phase(); broadcast_game_state()
        elif (phase == "Voting" and sub_phase == "Nomination" and now - start_time >= VOTING_NOMINATION_TIMER_SECONDS):
            process_nominations()
