...
Original file is located at
    https://colab.research.google.com/drive/1BbSLziW4WzR2BYkSQAiMQu-Ny9T4Y7c0
"""
...
Original file is located at
    https://colab.research.google.com/drive/1PPpx9SXPwyGFQI2Vl679guBDxWJ6x31I
"""
...
Original file is located at
    https://colab.research.google.com/drive/1pmVqP9loURnjMlHsKR0o-yadn2bmMlMO
"""

# -*- coding: utf-8 -*-
"""server.py (with False Idol)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14bQ8HD5LurA6kVKmLfY6XvqyN7IUUbQ7
"""
...
def handle_start_game_request(data=None):
    """Handles a player requesting to start the game."""
    sid = request.sid
    if game_state.current_phase != "Lobby" or game_state.game_setup_completed:
        emit('error', {"message": "Cannot start game now."}, room=sid)
        return
    ...
    if named_count != game_state.desired_players_count:
        remaining = game_state.desired_players_count - named_count
        emit('error', {"message": f"Waiting for {remaining} more players."}, room=sid)
        return

    game_state.game_setup_completed = True
    print("[GAME] Starting game logic.")
    start_game_logic()
...
def handle_submit_evening_cards(data):
    ...
    sacrifices = data.get("sacrifice_cards", [])
    targets = data.get("card_targets", {})

    valid = []
    for name in selected:
        c = player.get_card_by_name(name)
        if not c: return
        ...
        if game_state.current_phase not in c.phase_restriction and "Any" not in c.phase_restriction:
            emit('error', {"message": f"Cannot play {name} now."}, room=sid)
            return
        valid.append(c)

    for c in valid:
        player.remove_card(c.name)
        actual_sacrifices = []
        for s_name in sacrifices:
            s_card = player.get_card_by_name(s_name)
            if s_card:
        ...
        for s_card in actual_sacrifices:
            player.remove_card(s_card.name)

        apply_card_effect(pid, c, targets.get(c.name))
...
def handle_play_special_card(data):
    ...
    card = player.get_card_by_name(card_name)
...
def handle_confirm_cultist_kill(data=None):
    ...
    for cultist_id in game_state.cultist_kill_votes:
...
def handle_play_voting_card(data):
    ...
    apply_card_effect(pid, card, targets.get(card.name))
...
def apply_card_effect(player_id, card_obj, target_list=None):
    ...
    if card_obj.effect_type == "eternal_winter":
...
def resolve_dawn_actions():
    ...
    for action in game_state.delayed_actions:
        if action['execute_at_round'] == game_state.round_number:
            if action['type'] == 'steal_card_transfer':
                ...
                if thief and victim and victim.is_alive and victim.hand:
                    ...
                    victim.remove_card(stolen_card.name)
            ...
            elif action['type'] == 'peeping_tom_reveal':
                game_state.public_announcements.append(f"{action['peeper_name']} was slinking around last Night and caught a glimpse of {action['victim_name']}'s cards!")
                print(f"[EFFECT] {action['peeper_name']} was revealed as a Peeping Tom.")
    ...
    holy = game_state.global_status_effects.get("holy_water_protection_active", False)
    for action in list(game_state.pending_night_actions):
        ...
        if not action.get("is_countered", False):
            if action["effect_type"] == "kill":
                if "protected" in target_player.status_effects or "divine_protection" in target_player.status_effects or holy:
                    ...
                    kill_player(action['target_id'])
            elif action["effect_type"] == "silence":
...
def resolve_execution_vote():
    ...
    if len(tied_players) > 1:
        ...
        if 'divine_protection' in executed_player.status_effects:
            ...
            for voter_id, voted_id in game_state.voting_final_votes.items():
                if voted_id == executed_id:
...
def game_loop():
    ...
    while True:
        ...
        if game_state.current_phase == "Night":
            actions_to_remove = []
            for action in game_state.delayed_actions:
                if action['type'] == 'burn_death' and game_state.round_number >= action['execute_at_round']:
                    target_player = game_state.get_player(action['target_id'])
